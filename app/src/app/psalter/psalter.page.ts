...
@Component({
  ...
  animations: [
    trigger("swiping", [
      ...
      transition("none => left", [animate("100ms")]),
      transition("none => right", [animate("100ms")]),
      transition("left => right", [animate("0s")]),
...
export class PsalterPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.versionOptions$ = of([
      { value: "bcp1979", label: "1979 BCP" },
      { value: "coverdale", label: "Coverdale" },
      { value: "ip", label: "Inclusive Psalter" },
    ]);
    ...
    this.psalm$ = combineLatest([
      ...
      switchMap(([number, language]) =>
        this.documentService.findDocumentsBySlug(`psalm_${number}`, language)
      ),
      map((docs) => {
        const doc = docs.find(
          (doc) => doc?.version === this.version.value
        ) as Psalm;
        return new Psalm({
          ...doc,
          metadata: {
